...
export const api = {
  ...
  // Admin (passes stored secret so admin works from any logged-in account)
  _adminH: () => { const s = localStorage.getItem("pm_admin_secret"); return s ? { "X-Admin-Secret": s } : {}; },
  ...
  adminDeleteListing: (id) => req(`/api/admin/listings/${id}`, { method:"DELETE", headers: api._adminH() }),
  adminReports: (params) => req(`/api/admin/reports?${new URLSearchParams(params)}`, { headers: api._adminH() }),
  adminResolveReport: (id, payload) => req(`/api/admin/reports/${id}/resolve`, { method:"POST", body: payload, headers: api._adminH() }),
  adminReviews: (params) => req(`/api/admin/reviews?${new URLSearchParams(params)}`, { headers: api._adminH() }),
  adminDeleteReview: (id) => req(`/api/admin/reviews/${id}`, { method:"DELETE", headers: api._adminH() }),
  adminAds: () => req("/api/admin/ads", { headers: api._adminH() }),
  adminCreateAd: (payload) => req("/api/admin/ads", { method:"POST", body: payload, headers: api._adminH() }),
  adminUpdateAd: (id, payload) => req(`/api/admin/ads/${id}`, { method:"PUT", body: payload, headers: api._adminH() }),
  adminDeleteAd: (id) => req(`/api/admin/ads/${id}`, { method:"DELETE", headers: api._adminH() }),
};
